...
namespace SmartContractsGenerator.Model.Tests
{
    ...
    public class ContractFunctionTests
    {
        ...
        static IEnumerable<object[]> GenerateTestData()
        {
            ...
            var expectedWithModifierAndReturnsAndViewAndParamList2Payable = $"function {name1}({parametersCode2}) {visibility4.GenerateCode()} {m2Name}({parametersCode2}) {ModificationType.View.GenerateCode()} payable returns ({returningType2}) {{\n}}";
        ...
        static object[] GenerateRow(string parametersListCode, string name, string instructionsListCode, Visibility? visibility, Modifier m, string modifierParametersListCode, bool anyModifierParameter, string returningType, ModificationType modificationType, bool isPayable, string expected)
        {
